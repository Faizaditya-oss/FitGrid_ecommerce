...
if(
    ...
    if ($cart_result->num_rows > 0) {
        ...
        if ($quantity <= 0) {
            ...
            // Update quantity
            $update_item = "UPDATE cart_items SET quantity = ? WHERE cart_id = ? AND product_id = ? AND size = ? AND color = ?";
            $stmt_update = $db->prepare($update_item);
            $stmt_update->bind_param("iiiss", $quantity, $cart_id, $product_id, $size, $color);
            
            if ($stmt_update->execute()) {
                if ($stmt_update->affected_rows > 0) {
                    echo json_encode(array("success" => true, "message" => "Quantity updated."));
                } else {
                    echo json_encode(array("success" => false, "message" => "Product not found in cart or quantity is identical."));
                }
            } else {
